function dellModule() {
	...
	function addCCButton()
	{
		...
		// add the CC button
		$( this ).find( ".vod" ).append( _html );
		
		var _cc_icon = $( this ).find( "#cc_icon" );
		
		// bind the click handler to the CC button
		$( _cc_icon ).bind( "click", "", ccButtonHandler );
	}
	...
	function addCCPopup()
	{
		...
		var _captionToggle = $( this ).find( "#caption-toggle" );
		
		$( _captionToggle ).bind( "click", "", toggleCaptions );
		// find the on button
		var _onCaptionButton = $( this ).find( "#on-caption" );
		// bind the on button to the click
		$( _onCaptionButton ).bind( "click", "", onCaptions );
		// find the off button
		var _offCaptionButton = $( this ).find( "#off-caption" );
		// bind the on button to the click
		$( _offCaptionButton ).bind( "click", "", offCaptions );
		// set the videoLanguagePopup div
		videoLanguagePopup = $( this ).find( ".vod" ).find( ".videoLanguagePopup" );
		// hide the language popup
		$( videoLanguagePopup ).hide();
		
		return $( this ).find( ".vod" );
	}
	...
	function languageButtonHandler( event )
	{
		...
		playerObject.mb.publish( "showClosedCaptions" );
		// turn on the button
		var captionToggle = $(playerContainer).find("#caption-toggle");
		var isOff = captionToggle.hasClass("off");
		if( isOff )
		{
			$(captionToggle).toggleClass( "on" );
			$(captionToggle).toggleClass( "off" );
		}
	...
	function createCaptionsCollection( d )
	{
		...
		if ( _numCCLanguages > 0) 
		{
			...
			if( _cc_icon.length === 0 )
			{
				...
				_ccPopupLanguageDiv = $( _ccPopupParentDiv ).find( "#listLanguages" );
			}
		}
		else
		{
		...
		OO.plugin( "DellUIModule", function( OO, _, $, W ) {
			...
			Plugin.DellUIModule.prototype = 
			{
				...
				onPlayheadChanged: function( event, currentTime, duration, seekTime ) {	
					if( ctaContainer )
					{
						var _isVisible = $( ctaContainer ).css( "visibility" );
					...
					if( _metadata !== undefined && _metadata !== "" )
					{
						...
						if( ctaMetadataArray.length > 0 )
						{
							...
							if( hasBeenSet )
							{
								...
								// add the CC button
								$( ctaContainer ).append( _html );
								var _ctaButton = $( ctaContainer ).find( ".oo_cta_button" );
								// bind the click handler to the CC button
								$( _ctaButton ).bind( "click", "", function(){ playerObject.mb.publish( "ctaButtonClick", "ctaUI" ); } );
